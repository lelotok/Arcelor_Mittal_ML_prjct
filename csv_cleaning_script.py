...
for coil in coil_df.coil:
    for index in ["4", "5"]:
        try:
            with open(f"{filepaths.PATH_TO_OLD}{coil}B{index}.csv") as read_file:
            ...
            if captures != None:
                ...
                with open(f"{filepaths.PATH_TO_NEW}{coil}B{index}.csv", "w") as write_file:
...
for coil in bad_coils:
    if f"{coil}B4.csv" in final_coils:
        os.remove(f"{filepaths.PATH_TO_NEW}{coil}B4.csv")

coil_df = coil_df[coil_df.coil.isin(bad_coils) == False]

coil_df.to_csv(filepaths.PATH_TO_BASE_CSV)
